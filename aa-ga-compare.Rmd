...
summary_results <- function(metric){
  ...
  boxplot_gg <- ggplot(summary_set, aes(x = 0, y=Diffs)) + 
    geom_boxplot(fill='#A4A4A4', color="grey50") +
  ...
  boxplot_diffs <- ggplotly(boxplot_gg, width = 900, height = 150)
  ...
  actuals <- ggplotly(actuals_gg, width = 900, height = 300)
  
  # Now, create a chart that shows the difference between the
  # values by day in absolute terms.
  totaldiff_gg <- ggplot(data = master_diffs,
                         aes_string(x = "Date", y = paste0("`",metric," Total Diff`"))) +
    geom_line(color = "grey50") +
    labs(title = "Difference by Day (Adobe Analytics - Google Analytics)") +
    default_theme +
    scale_color_manual(values = "#DDDDDD") +
    scale_y_continuous(labels = comma)
  
  # Use Plotly to make it somewhat interactive
  totaldiff <- ggplotly(totaldiff_gg, width = 780, height = 200)
  ...
  # Use Plotly to make it somewhat interactive
  percentdiff <- ggplotly(percentdiff_gg, width = 780, height = 200)
...
The daily mean difference between the two platforms was **`r format(round(as.numeric(mean(processed_results$daily_diffs$Diffs)), 1), nsmall=1, big.mark=",")`**. The median difference was **`r format(round(as.numeric(median(processed_results$daily_diffs$Diffs)), 1), nsmall=1, big.mark=",")`**. The boxplot below shows the median, quartiles, and outliers.

```{r uvs charts, echo = FALSE, fig.align = "center"}
processed_results$boxplot_diffs
processed_results$linechart_actuals
processed_results$linechart_totaldiff
